    ...
    drawPath: function (renderContext) {
      ...
      if (this.vertexCodes.length === 0) {
        ...
        if (this.vertices[0].length === 2) {
          ...
          // drawing 3D vertices
          for (i = 0, j = this.vertices.length; i < j; i++) {
            renderContext.vertex(this.vertices[i][0], this.vertices[i][1], this.vertices[i][2]);
          }
        }
      } else {
    ...
    drawPrimitive: function (renderContext) {
      if (this.kind === PConstants.POINT) {
        ...
        if (this.params.length === 4) {
          // 2D
          renderContext.line(this.params[0], this.params[1], this.params[2], this.params[3]);
        } else {
          // 3D
          renderContext.line(
            this.params[0],
