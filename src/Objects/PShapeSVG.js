...
export default function (options) {
  ...
  PShapeSVG.prototype.parsePath = function () {
    ...
    var pathData = CommonFunctions.trim(
    ...
    while (i < pathData.length) {
      ...
      if ((valOf >= 65 && valOf <= 90) || (valOf >= 97 && valOf <= 122)) {
        ...
        if (i < pathData.length) {
          // don't go over boundary of array
          tmpArray = [];
          valOf = pathData[i].charCodeAt(0);
          while (
            !(
            ...
            // if its NOT a letter
            if (valOf === 32) {
              //if its a space and the str isn't empty
        ...
        if (valOf === 77) {
          ...
          // Z or z (these do the same thing)
          this.close = true;
        }
  ...
  PShapeSVG.prototype.parsePoly = function (val) {
    this.family = PConstants.PATH;
    this.close = val;
    var pointsAttr = CommonFunctions.trim(
