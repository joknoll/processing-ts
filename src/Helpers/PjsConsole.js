...
export default function PjsConsole(document) {
  var e = { BufferMax: 200 },
    style = document.createElement("style"),
    added = false;

  style.textContent = [".pjsconsole.hidden {", "  display: none!important;", "}"].join("\n");
  ...
  e.print = e.log = function () {
    ...
    var text = args
      .map(function (t, idx) {
        return t + (idx + 1 === args.length ? "" : " ");
      })
      .join("");
    if (e.BufferArray[e.BufferArray.length - 1])
      e.BufferArray[e.BufferArray.length - 1] += text + "";
    else e.BufferArray.push(text);
