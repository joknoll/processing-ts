const testHarness = (function testHarness() {
  ...
  Array.prototype.compareArrays = function (arr, eps) {
    ...
    for (var i = 0; i < arr.length; i++) {
      if (this[i].compareArrays) {
        // nested array?
        if (!this[i].compareArrays(arr[i], eps)) return false;
        else continue;
      }
      if ((!eps && this[i] != arr[i]) || (eps && Math.abs(this[i] - arr[i]) > eps)) return false;
    }
    return true;
  };
  ...
  var UnitTests = {
    ...
    _checkEqual: function (a, b) {
      ...
      if (a.compareArrays && b.compareArrays) {
        if (a.compareArrays(b, eps)) this._pass();
        else this._fail(a + " != " + b);
      } else {
        if ((!eps && a != b) || (eps && Math.abs(a - b) > eps)) this._fail(a + " != " + b);
        else this._pass();
      }
    },
    ...
    _checkNotEqual: function (a, b) {
      if (a.compareArrays && b.compareArrays) {
        if (a.compareArrays(b)) this._fail(a + " == " + b);
        else this._pass();
      } else {
        if (a == b) this._fail(a + " == " + b);
        else this._pass();
      }
    },
    ...
    _checkIsNaN: function (a) {
      if (a != a)
        // better check than isNaN()
        this._pass();
      else this._fail(a + " expected to be NaN.");
    },

    _checkIsNull: function (a) {
      if (a === null || a === undefined) this._pass();
      else this._fail(a + " expected to be null (or undefined).");
    },
    ...
    _checkThrows: function (f) {
      ...
      try {
        ...
        result = shouldThrow
          ? this._pass()
          : this._fail(f + " should not have thrown exception, but did.");
      }
    },
