/**
    ...
    Custom browser faking object for code-validation
    in automated test settings.

**/

const Browser = (function fakeBrowser() {
  ...
  var Canvas = function () {
    ...
    this.getContext = function () {
      return {
        ...
        getImageData: function () {
          return { width: 1, height: 1, data: [1, 2, 3, 4] };
        },
    ...
    this.hasAttribute = function (name) {
      return this.__attributes.hasOwnProperty(name.toLowerCase());
    };
    this.getAttribute = function (name) {
      return this.__attributes[name.toLowerCase()];
    };
    this.setAttribute = function (name, value) {
      this.__attributes[name.toLowerCase()] = value;
    };
  };
  ...
  function DOMParser() {
            ...
            default:
              ...
              for (q = 0; q < content.attributes.length; ++q) {
                if (content.attributes[q].name.substring(0, 6) === "xmlns:") {
                  scope.namespaces[content.attributes[q].name.substring(6)] = trim(
                    content.attributes[q].value,
                  );
                  ...
                  scope[content.attributes[q].name.substring(4)] = trim(
                    content.attributes[q].value,
                  );
    ...
    function NamedNodeMap(owner) {
      ...
      nodes.getNamedItemNS = function (namespaceURI, localName) {
        for (var i = 0; i < nodes.length; ++i) {
          if (nodes[i].localName === localName && nodes[i].namespaceURI === namespaceURI)
            return nodes[i];
        }
        return null;
      };
      nodes.setNamedItemNS = function (arg) {
        for (var i = 0; i < nodes.length; ++i) {
          if (nodes[i].localName === arg.localName && nodes[i].namespaceURI === arg.namespaceURI) {
            var old = nodes[i];
            nodes[i] = arg;
            old.ownerElement = null;
            return old;
  ...
  // and return our fake browser object
  return {
    isDomPresent: false,
    navigator: navigator,
    window: window,
